...
編集用の視点はShift＋ドラッグ／中ボタンドラッグで平行移動できます。「✥」をオンにすると通常ドラッグでも移動できます。ホイールで拡大縮小、「⌖」で視点をリセット。これらは出力カメラ・素材座標・書き出しに影響しません。

素材・効果パックの作成と別ウィンドウ配置の操作は docs/plugin-packs.md を参照してください。サンプルは examples/motion-pack.json です。
