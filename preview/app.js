...
function addMedia(a){if(!reserveClip())return;const track=selection()?.track??0;const start=Math.max(time,...project.clips.filter(c=>c.track===track).map(c=>c.start+c.duration));const c=C.createClip(crypto.randomUUID(),a.kind==='model'?'model':'media',start,track,a.id);c.duration=a.duration;project.clips.push(c);selected=c.id;time=start;changed();revealTrack(track);}
...
function setView(){
 fitViewport();
 $('#viewport').classList.toggle('spatial',spatial);$('#mode3d').classList.toggle('active',spatial);$('#mode2d').classList.toggle('active',!spatial);$('#composition').style.transform=`translate(${viewPanX}px,${viewPanY}px) scale(${viewZoom})`;$('#viewZoomLabel').textContent=Math.round(viewZoom*100)+'%';$('#viewLabel').textContent=spatial?'/ 3D 空間':'/ 出力画面';$('.viewportlabel').firstChild.textContent=spatial?'空間編集 ':'出力プレビュー ';$('.viewportlabel span').textContent=spatial?'レイヤー優先':`${project.width||3840} × ${project.height||2160}`;$('.viewhint').textContent=spatial?'ドラッグで視点回転 · Shift／中ボタンドラッグで視点移動 · ホイールで拡大縮小 · Alt＋ドラッグで素材移動':'素材をドラッグで移動 · Shift／中ボタンドラッグで視点移動 · ホイールで拡大縮小';renderFrame();
}
...
$('#clearEffect').onclick=()=>{const c=selection();if(!c)return;checkpoint();c.effects={};c.effectName='';changed();};$('#installPlugin').onclick=()=>$('#pluginInput').click();$('#pluginInput').onchange=async e=>{try{const file=e.target.files[0];if(file.name.endsWith('.py')){const r=await fetch('/api/plugin/python',{method:'POST',headers:{'X-Lattice-Token':token,'X-Filename':encodeURIComponent(file.name)},body:file});const res=await r.json();if(!r.ok)throw Error(res.error);toast('Pythonプラグインをインストールしました。再起動後に有効になります。');}else{await api('/api/plugin',JSON.parse(await file.text()));await refreshState();toast('プラグインを読み込みました。');}}catch(error){toast(error.message);}finally{e.target.value='';}};
