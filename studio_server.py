...
def probe(path):
    if path.suffix.lower() in {'.glb', '.gltf', '.obj', '.pmx', '.pmd', '.vmd'}:
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=='/api/import':
                ...
                if suffix not in {'.mp4','.mov','.mkv','.webm','.avi','.m4v','.wav','.mp3','.aac','.m4a','.flac','.ogg','.glb','.gltf','.obj','.pmx','.pmd','.vmd'}: raise ValueError('対応していない拡張子です。')
